...
"Traduzir Textos:"
"Corrigir o ID e a Palavra-Chave"
"Testar:"
...
def validaCampo(): # Valida se o Campo de Busca da API está preenchida
    
    # Vincular o Objeto de Tela a Variável
    ConselhoAleatorio = TxtConselhoAleatorio.text()
    ConselhoBusca = TxtConselhoBusca.text()
    conselhoID = TxtConselhoID.text()
    Nome = TxtNome.text()
    # Fazer Validação de Preenchimento dos Campos
    if Nome == "":
        QMessageBox.critical(TelaConselho, "Atenção", "Nome precisa ser informado, verifique!")
        TxtNome.setFocus()

    else:
        # Chamar a Rotina de Verificação do Conselho
        tratarConselhoAleatorio()
...
def tratarConselhoBusca():
    ...
    if TxtConselhoBusca.text() == "":
        QMessageBox.information(TelaConselho, "Atenção", "Digite algo para Buscarmos")
